...
export default function Footer() {
  ...
  return (
    <footer className="section-stack-lg pb-12 overflow-hidden border-t border-[var(--color-border)]" style={{ backgroundColor: "var(--color-bg)" }}>
      <div className="container-sophep">
        <div className="mb-16 md:mb-24">
          <h2
            className="font-display leading-[0.9] tracking-[-0.03em] font-bold select-none transition-colors duration-500 text-balance"
            style={{ color: "var(--color-fg)", fontSize: "clamp(2.8rem, 11vw, 12rem)" }}
          >
            <GlitchHeading text="SOPHEP." intervalMs={6000} />
          </h2>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10 md:gap-14 lg:gap-16 mb-20 md:mb-28">
          <FooterColumn title="Platform" links={footerLinks.platform} />
          <FooterColumn title="Events" links={footerLinks.events} />
          <FooterColumn title="Join Us" links={footerLinks.registration} />
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-6">
              <button className="group flex flex-col gap-1 text-left" onClick={() => copyToClipboard("sophep@giki.edu.pk")}>
            ...
            <div className="flex flex-col gap-4">
              {["Instagram", "X", "Linkedin", "GitHub"].map((social) => (
                <Magnetic key={social} strength={0.2}>
                  <a href="#" className="font-sans text-[13px] text-[var(--color-fg-muted)] hover:text-[var(--color-primary)] transition-all uppercase tracking-widest">
                    {social}
                  </a>
                </Magnetic>
              ))}
        ...
        <div className="mb-16 md:mb-20 pt-10 border-t border-[var(--color-border)]">
            ...
            <div className="w-full md:w-auto flex-grow max-w-md">
              <form className="relative flex items-center" onSubmit={(e) => e.preventDefault()}>
                <input
                  ...
                  className="w-full bg-[var(--color-bg-muted)]/10 border border-[var(--color-border)] rounded-full py-4 pl-6 pr-28 sm:pr-32 font-sans text-sm text-[var(--color-fg)] placeholder:text-[var(--color-fg-muted)]/50 focus:outline-none focus:border-[var(--color-primary)]/50 transition-colors"
                  required
                />
                <button
                  type="submit"
                  className="absolute right-2 top-2 bottom-2 bg-[var(--color-primary)] text-[var(--color-bg)] font-sans text-xs uppercase tracking-widest font-bold px-4 sm:px-6 rounded-full hover:bg-[var(--color-primary)]/90 transition-colors"
                >
                  Subscribe
                </button>
              </form>
        ...
        <div className="pt-10 border-t border-[var(--color-border)] flex flex-col md:flex-row items-center md:justify-between gap-6 md:gap-8">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-12">
            {["Privacy", "Terms", "Imprint"].map((item) => (
              <Link key={item} href="#" className="font-sans text-[12px] text-[var(--color-fg-muted)] hover:text-[var(--color-primary)] transition-opacity">
                {item}
              </Link>
            ))}
          </div>
...
function FooterColumn({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        {links.map((link) => (
          <Link key={link.label} href={link.href} className="font-sans text-[13px] text-[var(--color-fg-muted)] hover:text-[var(--color-primary)] transition-all">
            {link.label}
          </Link>
        ))}
      </div>
